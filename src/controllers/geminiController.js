...
// Simple in‑memory cache: { userId: Map(productId -> { recipe, expiryDate }) }
...
/**
 * Generate short, playful recipes in ENGLISH for Food products
 * expiring within 7 days, using DeepSeek via OpenRouter.
 * Uses a simple per-user, per-product cache so recipes are
 * not regenerated on every refresh.
 */
export const getRecipeSuggestion = async (req, res) => {
  try {
    const userId = req.user._id;           // ObjectId for DB query
    const userKey = userId.toString();     // string key for cache

    // Ensure this user has a cache map
    ...
    // Limit how many recipes we generate per request
    const MAX_RECIPES = 3;
