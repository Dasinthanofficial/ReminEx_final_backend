...
const OCR_API_KEY = process.env.OCR_SPACE_KEY; // set in .env if you use OCR
...
export const scanLabelImage = async (req, res) => {
  try {
    ...
    if (!OCR_API_KEY) {
      return res
        .status(500)
        .json({ message: "OCR API key not configured" });
    }

    // Build multipart/form-data using in‑memory buffer from multer.memoryStorage()
    const form = new FormData();
    form.append("file", req.file.buffer, {
      filename: req.file.originalname || "label.jpg",
