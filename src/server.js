...
// ---------------------------------------------------------------
// 5️⃣ Generic middleware (INCREASED LIMIT FOR IMAGES)
// ---------------------------------------------------------------
app.use(
  cors({
    origin: process.env.CLIENT_URL || "http://localhost:5173",
...
// 👇 Updated limits to 50mb to allow sending images in emails
...
app.use((err, req, res, next) => {
  ...
  if (err.type === 'entity.too.large') {
    return res.status(413).json({ message: "File/Email too large (Max 50MB)" });
  }
